...
time_ma = np.array(list(range(1 * 10**6, 4600 * 10**6,
                              1 * 10**6)))  # 1-4601 Ma
    ...
    if os.path.exists(outfile):
        if ("pdf" in driver):
            ...
            if not (opt_force_overwrite):
                ...
                if ((len(answer) != 0)
                        and (answer or answer[0].lower()) == 'y'):
...
def oneWM(X, s1, conf):
    ...
    sm = stats.norm.ppf(conf + (1 - conf) / 2.) * np.sqrt(1. / np.sum(s1**
                                                                      (-2)))
...
def ConcAgeConv(Xi, Yi, sigma_Xi, sigma_Yi, rhoXYi, Tinit=10.**6, conf=0.95):
    ...
    # eq(3)
    o11 = sigma_Y_bar**2 / (
        (sigma_X_bar**2) * (sigma_Y_bar**2) - cov_XY_bar**2)
    # eq(3)
    o22 = sigma_X_bar**2 / (
        (sigma_X_bar**2) * (sigma_Y_bar**2) - cov_XY_bar**2)
    # eq(3)
    ...
    S_bar = FitFuncConv(T_leastsq, X_bar, Y_bar, sigma_X_bar, sigma_Y_bar,
                        rho_XY_bar)
...
def ConcAgeTW(Xi, Yi, sigma_Xi, sigma_Yi, rhoXYi, Tinit=10.**6, conf=0.95):
    x = U85r * Yi / Xi
    y = 1. / Xi
    sigma_x = np.sqrt(((sigma_Xi / Xi)**2 + (sigma_Yi / Yi)**2 - 2 *
                       (sigma_Xi / Xi) * (sigma_Yi / Yi) * rhoXYi)) * x
    sigma_y = sigma_Xi / Xi * y
    rhoxy = ((sigma_Xi / Xi)**2 - (sigma_Xi / Xi) *
             (sigma_Yi / Yi) * rhoXYi) / ((sigma_x / x) * (sigma_y / y))
...
def SlopeIntercept_LS(x, y):
    ...
    result = optimize.leastsq(
        FitFuncSI_LS, init_ab, args=(x, y), full_output=1)
...
def SlopeIntercept(x, y, sigma_x, sigma_y, rho_xy, case):
    ...
    if (case == 1):
        bi1 = optimize.leastsq(
            FitFuncSI, init_b, args=(x, y, wx, wy, rho_xy, case))[0][0]
...
def SIsigma2(x, x_bar, y_bar, b, sigma_a, sigma_b, conf=0.95):
    ...
    b2 = (np.tan(np.arctan(b) + dtheta) + np.tan(np.arctan(b) - dtheta)) / 2.
    a2 = y_bar - b2 * x_bar
    sigma_b2 = (
        np.tan(np.arctan(b) + dtheta) - np.tan(np.arctan(b) - dtheta)) / 2.
...
def SIageConv(a, b, sigma_a, sigma_b, x_bar, y_bar, init_t=10**6, conf=0.95):
    T = optimize.leastsq(
        FitFuncSIageConv,
        init_t,
        args=(a, b, sigma_a, sigma_b, x_bar, y_bar, conf, 0))[0][0]
    Tmin = optimize.leastsq(
        FitFuncSIageConv,
        init_t,
        args=(a, b, sigma_a, sigma_b, x_bar, y_bar, conf, 1))[0][0]
    Tmax = optimize.leastsq(
        FitFuncSIageConv,
        init_t,
        args=(a, b, sigma_a, sigma_b, x_bar, y_bar, conf, 2))[0][0]
    if Tmax < Tmin:
        Tmax, Tmin = Tmin, Tmax
    return (T, Tmin, Tmax)
...
def discordance(age_7Pb_5U, age_7Pb_5U_se, age_6Pb_8U, age_6Pb_8U_se,
                age_7Pb_6Pb, age_7Pb_6Pb_min, age_7Pb_6Pb_max, sd, method):
    ...
    if (method == 0):
        ...
        disc_percent = (1 - ((age_7Pb_5U - age_7Pb_5U_se * sd) /
                             (age_6Pb_8U + age_6Pb_8U_se * sd))) * 100.0
    else:
...
def plot_data_point_error_ellipse(ax, axn, X, Y, sigma_X, sigma_Y, cov_XY, cr,
                                  ind, outd, outd_disc):
    for i in range(len(X)):
        if (i in ind):
            dp_ell = myEllipse(
...
def plot_2D_wm(ax, axn, X, Y, sigma_X, sigma_Y, rho_XY, cr, legend_pos_x,
               legend_pos_y):
    Xwm_bar, Ywm_bar, MSWDwm, sigma_Xwm_bar, sigma_Ywm_bar, cov_XYwm_bar = \
        twoWM(X, Y, sigma_X, sigma_Y, rho_XY, conf=cr)
    twm_ell = myEllipse(
        0,
        Xwm_bar,
        Ywm_bar,
    ...
    ax[axn].text(
        legend_pos_x,
        legend_pos_y,
        '2D weighted mean [%d%% conf.] (MSWD=%s)' % (cr * 100,
                                                     format(MSWDwm, dignum)),
        transform=ax[axn].transAxes,
        verticalalignment='top',
        fontsize=legend_font_size)
...
def plot_concordia_age(ax, axn, T_lsq, S_lsq, X_lsq, Y_lsq, MSWD, cr,
    ...
    ax[axn].text(
        legend_pos_x,
        legend_pos_y,
        u'Concordia age = %s ±%s %s [%d%% conf.]' % (format(
            T_lsq / age_unit, dignum), format(S_lsq / age_unit, dignum),
...
def plot_concordia_age_MSWD(ax, axn, MSWD, ca_mswd, p_value, legend_pos_x,
                            legend_pos_y):
    ...
    if (ca_mswd == 1):
        ax[axn].text(
            legend_pos_x,
            legend_pos_y,
            r'(MSWD of equivalence = %s, p($\chi^2$) = %s)' % (format(
                MSWD, dignum), format(p_value, dignum2)),
            transform=ax[axn].transAxes,
            verticalalignment='top',
            fontsize=legend_font_size)
    elif (ca_mswd == 2):
        ax[axn].text(
            legend_pos_x,
            legend_pos_y,
            r'(MSWD of combined = %s, p($\chi^2$) = %s)' % (format(
                MSWD, dignum), format(p_value, dignum2)),
            transform=ax[axn].transAxes,
            verticalalignment='top',
            fontsize=legend_font_size)
        ...
        ax[axn].text(
            legend_pos_x,
            legend_pos_y,
            r'(MSWD of concordance = %s, p($\chi^2$) = %s)' % (format(
                MSWD, dignum), format(p_value, dignum2)),
            transform=ax[axn].transAxes,
            verticalalignment='top',
            fontsize=legend_font_size)
...
def plot_concordia_intercept_age(ax, axn, ctype, X, Y, sigma_X, sigma_Y, cr,
                                 rho_XY, range_XY, T_lsq, case, legend_pos_x,
                                 legend_pos_y):
    ...
    ax[axn].fill_between(
        ...
        interpolate=True)
    ax[axn].plot(
        xx, bi * xx + ai, linewidth=ia_line_width, color=ia_line_color)
    ...
    ax[axn].text(
        ...
        (format(Tsi / age_unit, dignum), format(
            (Tmax - Tsi) / age_unit, dignum),
         format((Tmin - Tsi) / age_unit, dignum), age_unit_name, cr * 100),
        transform=ax[axn].transAxes,
        verticalalignment='top',
        fontsize=legend_font_size)
...
def plot_oneD_weighted_mean(ax, axn, oneD_age_type, Tall, s1, ind, outd,
                            outd_disc, cr, legend_pos_x, legend_pos_y):

    Twm, sm, MSWD = oneWM(Tall[ind], s1[ind], conf=cr)

    # confidence band of the weighted mean
    ax[axn].axhspan(
        Twm - sm, Twm + sm, facecolor=oneD_band_fc, alpha=oneD_band_alpha)
    ax[axn].plot(
        [0., len(Tall) + 1], [Twm, Twm],
    ...
    ax[axn].text(
        ...
        (format(Twm, dignum), format(sm, dignum), age_unit_name, cr * 100,
         format(MSWD, dignum)),
        transform=ax[axn].transAxes,
        verticalalignment='top',
        fontsize=legend_font_size)
...
if __name__ == '__main__':
    ...
    range_xy = loads(config.get('Graph',
                                'range_xy_tw'))  # [[xmin,xmax],[ymin,ymax]]
    ...
    opt_concordia_ia = config.getboolean('Graph',
                                         'opt_concordia_intercept_age')
    ...
    dt_name_column = [
        '7Pb_5U', '7Pb_5U_1s', '6Pb_8U', '6Pb_8U_1s', '7Pb_6Pb', '7Pb_6Pb_1s'
    ]
    ...
    (age_7Pb_6Pb,
     age_7Pb_6Pb_se_plus, age_7Pb_6Pb_se_minus) = calc_age_7Pb_6Pb(
    ...
    # discordance
    if (opt_exclude_disc):
        print('Discordant data (> %s%%) are excluded from analysis.' % format(
            disc_thres, dignum))
        ...
        for i in outd_disc:
            print('%d: %s%% = (1-%.1f/%.1f) x 100' %
                  (i, format(disc_percent[i], dignum),
    ...
    if (rho_XY[ind].max() > 1):
        ...
        [
            print('%d %.2f' % (i, rho_XY[i])) for i in range(len(rho_XY))
            if rho_XY[i] > 1
        ]
    ...
    for i in range(len(y)):
        ...
        print('%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s' %
              (i, format(age_6Pb_8U[i] / age_unit, dignum),
    ...
    if (plot_diagrams[0] == 1):
        ...
        legend_pos = 0
        legend_data_number(ax, axn, legend_pos_x[legend_pos],
        ...
        if (opt_2D_wm):
            print('    Error ellipse is %d%% for 2D weighted mean' %
                  (twm_ee_cr * 100))
            legend_pos += 1
            plot_2D_wm(ax, axn, X[ind], Y[ind], sigma_X[ind], sigma_Y[ind],
                       rho_XY[ind], twm_ee_cr, legend_pos_x[legend_pos],
                       legend_pos_y[legend_pos])
        ...
        if (opt_concordia_ia):
            if (concordia_ia_case_cc == 0) or (concordia_ia_case_cc == 2):
                ...
                Tsi, Tmax, Tmin = plot_concordia_intercept_age(
                    ax, axn, 'conv', X[ind], Y[ind], sigma_X[ind],
                    sigma_Y[ind], ia_cr, rho_XY[ind], range_XY, T_lsq, case,
                ...
                print('    Intercept age = %s +%s %s %s [%d%% conf.]' %
                      (format(Tsi / age_unit, dignum),
                       ...
                       format((Tmin - Tsi) / age_unit, dignum), age_unit_name,
                       ia_cr * 100))
    ...
    if (plot_diagrams[1] == 1):
        ...
        if (opt_concordia_ia):
            if (concordia_ia_case_tw == 0) or (concordia_ia_case_tw == 2):
                ...
                case = 0
                Tsi, Tmax, Tmin = plot_concordia_intercept_age(
                    ax, axn, 'tw', x[ind], y[ind], sigma_x[ind], sigma_y[ind],
                    ia_cr, rho_xy[ind], range_xy, T_lsq, case,
                    legend_pos_x[legend_pos], legend_pos_y[legend_pos])
                print(('    Intercept age = %s +%s %s %s [%d%% conf.]' %
                       (format(Tsi / age_unit, dignum),
                        format((Tmax - Tsi) / age_unit, dignum),
    ...
    if (plot_diagrams[2] == 1):
        ...
        T_owm, S_owm, MSWD_owm = plot_oneD_weighted_mean(
            ax, axn, oneD_age_type, Tall, s1, ind, outd, outd_disc, oneD_cr,
            legend_pos_x, legend_pos_y)
        print(u'    1D weighted mean age = %s ±%s %s [%d%% conf.] (MSDW=%s)' %
              (format(T_owm, dignum), format(S_owm, dignum), age_unit_name,
               oneD_cr * 100, format(MSWD_owm, dignum)))
